...
段階 8 で当初の要件（F1〜F5）を作り切った。以降の改善タスクは GitHub Issues で管理する。設計書に「次の段階への申し送り」を積むやり方は段階 8 で終了した。

- 未対応の項目を見つけたら Issue を立てる。設計書の本文に書き足さない
- 対応しないと判断したものも `deferred` ラベルで残す。判断の経緯を追えるようにするため
- PR では `Closes #N` で紐づける
- ラベルは `before-release`（受講生に配る前に必要）、`performance`、`security`、`enhancement`、`deferred`

設計書は「今どうなっているか」を書く場所として保つ。「これからどうするか」は Issues に置く。
