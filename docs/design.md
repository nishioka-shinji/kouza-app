...
LIFF の初期化手順は段階 4 で、Webhook と応答メッセージの仕様は段階 5 で、LIFF の URL 解決（2 段階リダイレクトと `liff.state`）は段階 6 で、Access のワイルドカードの及ぶ範囲と `maxlength` の効き方は段階 7 で確認した（7 章）。いずれも 8 章の表に追記済み。
