...
応答メッセージなので課金対象外（6 章）。Webhook が生きていることの確認にもなる。

段階 8 でもこの分岐は広げなかった。F4 の入口をリッチメニューの postback に限ったため、テキストを解釈する必要がない。定型文に「出欠の連絡はメニューから」の 1 文を足すに留めている。
...
- 画面は「殻」と「中身の API」に分ける（下記）。`<form>` のマークアップは素のまま保ち、送信だけ `fetch` に載せ替える。クライアント側のフレームワークは使わない
...
素の form で POST する構成は取れない。ID トークンは `liff.getIDToken()` でしか取れず、これはブラウザで `liff.init()` が終わった後にしか手に入らないため、サーバーは最初の GET を受けた時点で送信者を特定できない。hidden input に埋めるにしても値を入れるのは JS であり、一覧の GET では hidden input 自体が置けない。

そのため 2 段構えにする。

| 段 | パス | 検証 | 返すもの |
| --- | --- | --- | --- |
| 殻 | `/liff/images`、`/liff/images/{uuid}` | なし | 空のコンテナと初期化スクリプトだけの HTML |
| 中身 | `/liff/api/images`、`/liff/api/images/{uuid}` | あり | その受講生のデータを含む HTML 断片 |

殻の JS が `liff.init()` → `getIDToken()` と進み、`Authorization: Bearer` を付けて中身を取りに行って差し込む。GET と POST で経路が揃い、トークンが URL に載らず、1 時間の失効もページの再読み込みだけで解消する。

`<form>` のマークアップは素のまま保ち、`submit` を `preventDefault` して `FormData` を送る。「クライアント側のフレームワークを使わない」という方針はこれで満たされる。
...
- 講座回を管理画面から登録する
- 受講生が LINE のリッチメニューから出席・欠席・遅刻を連絡する
- D1 に記録し、管理画面で回ごとに一覧する

Messaging API のリッチメニューやクイックリプライを試す場として位置づける。優先度は F1〜F3 より低い。

どの回への連絡かは、リッチメニューを起点とした 3 段の postback で特定する（段階 8 で確定）。

1. リッチメニューの「出欠連絡」で `action=attendance` が飛ぶ
2. 講座回をクイックリプライで選ばせ、`action=attendance&lesson_id={id}` が飛ぶ
3. 出欠をクイックリプライで選ばせ、`action=attendance&lesson_id={id}&status={...}` が飛ぶ

クイックリプライの選択肢は 13 件が上限のため、講座回は開催日の新しい順に 10 件へ絞る。

テキストによる連絡は実装しない。「欠席」のような文言を解釈するには会話の状態（どの回の話をしているか）を持つ必要があり、段階 8 の規模を超えるため。入口はリッチメニューだけにする。
...
F1 の返信も F4 の出欠連絡の応答も応答メッセージなので課金されない。200 通を消費するのは F5 のお知らせ配信だけ。

**通数は「リクエスト数 × 受信者数」で数える**（段階 8 で公式ドキュメントに確認）。ブロードキャストを 1 回送っても「1 通」ではなく、友だちの人数ぶん消費する。友だちが 10 人なら 1 回の配信で 10 通。ブロック中のユーザーと存在しないユーザー ID は対象から外れ、カウントもされない。

受講生が数人の個人開発では無料枠に収まるが、配信のたびに人数ぶん減ることは管理画面にも明示する。`GET /v2/bot/message/quota/consumption` で当月の消費通数を取れるので、配信前に画面へ出す。

2026 年 10 月 1 日に追加メッセージ料金の改定が予定されているが、これはスタンダードプランの超過分単価の話で、コミュニケーションプランには影響しない。
...
| 8 | 出欠とお知らせ | リッチメニューから出欠を連絡でき、管理画面で回ごとに一覧できる。管理画面からお知らせを配信できる |

段階 5 と 6 が習得の本体。段階 8 は当初「余力があれば」としていたが、着手して完了した。
...
`students.status` は `enrolled` / `completed`、`attendances.status` は `present` / `absent` / `late` を格納し、CHECK 制約で縛る。5 章の表記は「受講中／修了」「出席／欠席／遅刻」だが、これは画面での表示にあたる。SQL やコード中に日本語リテラルが散るのを避けるため、DB には英語を入れて表示側で和訳する。

対訳表は使う側に応じて置き場を分ける。`students.status` は管理画面でしか使わないので `src/admin/format.ts`、`attendances.status` は webhook と管理画面の両方から使うので `src/attendance.ts` に置く。webhook から `src/admin` 配下を import させないため（`src/format.ts` を作ったのと同じ理由。段階 7 参照）。
...
ただしこの結合は自動では起きない。エンドポイント URL が `liff.init()` を実行する HTML を返すことが条件になる（段階 6 で判明。8 章の「LIFF の 2 段階リダイレクト」参照）。
...
### 段階 6 の実施結果（2026-09-15）

F2 と F3 を実装した。構成は `src/line/verify.ts`（ID トークンの検証）、`src/liff/auth.ts`（検証のミドルウェア）、`src/liff/images.tsx`（一覧・メモ編集・保存）、`src/images/index.ts`（R2 からの画像配信）に分けた。段階 4 の疎通確認ページは役目を終えたので削除した。

#### 素の form で POST する構成は取れなかった

F2 の当初の記述を実装中に変更した。理由は F2 に書いたとおりで、ID トークンが `liff.init()` の後にしか取れないため、サーバーは最初の GET の時点で送信者を特定できない。殻と中身の API に分ける構成にした。

`<form>` のマークアップは素のまま保ち、`submit` を乗っ取って `FormData` を送る形にしたので、「クライアント側のフレームワークを使わない」という方針は維持している。

#### エンドポイント URL で `liff.init()` を実行する

段階 6 でいちばん時間を取られた点。実機で LIFF URL のパスが失われ、常に一覧に着地する事象が出た。

原因は、段階 4 の疎通確認ページを削除してエンドポイント URL（`/liff`）が 404 になったのを、302 リダイレクトで塞いだこと。LIFF はパスを `liff.state` に畳んでエンドポイント URL へ渡し、そこで実行された `liff.init()` が展開して本来の URL へ飛ばす。302 を返すと `init()` が走らないため、`liff.state` が読まれないまま破棄される。公式リファレンスは `init()` の resolve 前に 301 / 302 を返すことを明示的に禁止している。

外部ブラウザでのログインループも同じ原因だった。`init()` の前にリダイレクトされると、SDK が付与する `access_token` 等も失われて初期化が完結しない。最初は `liff.login()` の重複呼び出しを疑って削除したが、それでは直らなかった。

直し方は、エンドポイント URL が 302 ではなく `liff.init()` を実行する HTML を返すこと。`liff.state` があれば SDK が本来のパスへ飛ばし、無ければ（素の LIFF URL）一覧へ送る。パスで画面を分ける構成自体は仕様上問題なく、変更していない。

公式ドキュメントを先に読んでいれば避けられた。404 を塞ぐ判断は正しかったが、手段を確かめずに選んだ。

#### 所有者検証は SQL で絞る

`WHERE images.id = ? AND images.student_id = ?` として、不一致と不存在をまとめて 401 に潰す。存在の有無を漏らさないため。判定に使うのは検証済みの `c.get('lineUserId')` のみで、クライアント由来の値は使わない。

#### `JSON.stringify` は `</script>` をエスケープしない

画像 ID をインラインスクリプトに埋めるため、`</script><img src=x>` のような ID で script 要素から脱出できる状態だった。`<` を `<` に潰して塞いだ。実装者が指示外で見つけて対処した箇所。

#### 絞り込みは双方向にする

未メモのみ表示から全件へは戻れるが、逆がなかった。実機で気づいた。状態表示を対称にして往復できるようにした。

なお `<a href>` での遷移はページを読み込み直すため、そのたびに `liff.init()` が走る。`fetch` で中身だけ差し替えれば初期化は初回のみで済むが、快適さの改善であり完了条件に関わらないため見送った。

#### 確認できたこと

実機で完了条件をすべて満たした。

- 画像 3 件が一覧に並び、R2 から配信される
- 講座回（`2026-09-20` 第一回）を選んでメモを保存でき、本番 D1 に `lesson_id` と `memo_body` が入る
- 再度開いて編集でき、上書きされる（履歴は残らない）
- `memo=none` の絞り込みが効き、メモを書いた画像が一覧から外れる
- 他人の画像 ID を指定すると 401、同じ形式の URL で自分の画像は開ける

401 の確認には本番 D1 にダミーの受講生と画像を 1 件ずつ入れ、確認後に削除した。本番に他人のデータが存在しないため。

外部ブラウザでのログイン導線も動作した。

### 段階 7 の実施結果（2026-09-15）

F5 の「画像とメモの閲覧」を実装した。あわせて段階 6 の申し送り 2 件（日時が UTC のまま、`memo_body` に長さ上限がない）を回収した。構成は `src/admin/images.tsx`（一覧と絞り込み、管理画面用の配信）、`src/format.ts`（日時の整形）で、既存の `src/images/index.ts` から配信の本体を切り出して共有する形にした。

#### 一覧は 1 枚にして両軸で絞り込む

受講生詳細ページは作らず、`/admin/images` の 1 枚に受講生と講座回のセレクトを置いた。完了条件の「受講生ごと・講座回ごと」を 1 画面で満たせるため。受講生一覧・講座回一覧の各行からは、絞り込み済みの状態へリンクする。

ページネーションは入れていない。現状の画像は数件で、全件表示で足りる。

#### 管理画面の画像は Access 配下の別経路から配信する

`<img>` が参照するのは `/admin/images/{uuid}`。配信の本体（`serveImage`）を関数として切り出し、無認証の `/images/{uuid}` と共有する。受講生向けの経路は F3 で合意済みの割り切り（UUID の推測困難性に頼る）のまま変更していない。

Access の宛先追加は不要だった。既存の `/admin/*` が `/admin/images` と `/admin/images/{uuid}` の両方を覆う。8 章の落とし穴（`/admin/*` は `/admin` 自身を覆わない）は `/admin` が別途登録済みなので影響しない。デプロイ後に実機で 302 を確認した。一覧ページだけでなく画像 URL 側も確認している。ここが覆っていなければ画像が無認証で露出するため。

#### 日時は表示だけを JST に変換する

D1 の `CURRENT_TIMESTAMP` は UTC で入るため、画面が日本時間と 9 時間ずれていた。`formatJst` を `src/format.ts` に置き、管理画面と LIFF の両方から呼ぶ。DB の値は変更せず、SQL の並び順も UTC 文字列のままで正しい。

`lessons.held_on` は変換してはいけない。`YYYY-MM-DD` の日付文字列であって UTC タイムスタンプではないため、通すと開催日が全件 1 日前にずれる。

`src/admin/format.ts` ではなく `src/format.ts` に置いたのは、LIFF から管理画面のモジュールを import させないため。`src/admin` 配下は admin 専用という構造を保つ。

#### メモの上限は 2000 文字

D1 の行サイズ上限に当たると 500 になるため、サーバー側で弾く。`<textarea>` の `maxlength` は第一の防波堤で、通常のブラウザ操作では貼り付けが切り詰められてサーバーの検証には到達しない。サーバー側はフォームを介さない POST のための第二の防波堤になる。

400 で返すときは送信された値を差し戻す。DB から読んだ行をそのまま描画すると、利用者が書いた本文が消える。既存の講座回エラーの分岐も同じ欠陥を持っていたため揃えて直した。

#### 確認できたこと

デプロイ後に実機で確認した。

- 受講生ごと・講座回ごとの絞り込みが両軸とも効く。セレクトで「すべて」を選ぶ操作を含む
- 画像が管理画面に表示され、日時が JST で出る
- 受講生一覧・講座回一覧の「画像」リンクから絞り込み済みの一覧へ着地する
- `/admin/images` と `/admin/images/{uuid}` が未認証で Access のログイン画面へ 302
- 既存の `/images/{uuid}` が 200 で実バイナリを返す（配信の切り出しで壊れていない）
- `/webhook` が署名なしで 401（Access に巻き込まれていない）

絞り込みの確認には本番 D1 にダミーの受講生・講座回・画像を 1 件ずつ入れ、確認後に削除した。本番に受講生と講座回が 1 件ずつしかなく、絞り込めている証拠にならないため。削除後に件数が投入前へ戻ることも確認している。

#### 確認できていないこと

メモ 2000 文字の上限は、サーバー側の検証を本番の HTTP 経路で動かせていない。`maxlength` がブラウザ側で切り詰めるため通常操作では到達せず、到達させるには有効な ID トークンを付けた POST が要る。ID トークンは LINE アプリ内でしか発行できない。

実装はレビューで確認済みで、400 の応答で送信値が差し戻されることもコンポーネントのレンダリングで検証しているが、実経路を通した確認ではない。

#### 段階 8 への申し送り

`ORDER BY images.received_at` にインデックスがなく、絞り込みなしの一覧は `SCAN images` と TEMP B-TREE ソートになる。D1 はスキャンした行数で課金されるため、画像が数千件に育つと一覧のたびに全件スキャンになる。5 章のインデックス表を変える話なので、ページネーションを入れる段階で表ごと見直す。

メモの文字数は UTF-16 コード単位で数えている。`maxlength` も同じ基準なので両者にずれはないが、サロゲートペア（一部の漢字・絵文字）は 1 文字が 2 と数えられ、体感の半分で上限に当たる。実害は「2000 字まで書けない」だけで、現状維持とした。

### 段階 8 の実施結果（2026-09-16）

F4（出欠連絡）と F5 の残り 2 つ（出欠一覧、お知らせ配信）を実装した。当初「余力があれば」としていた段階だが、着手して完了させた。

構成は `src/attendance.ts`（出欠の対訳）、`src/webhook/attendance.ts`（出欠の保存）、`src/admin/attendances.tsx`（出欠一覧）、`src/admin/notices.tsx`（お知らせ配信）、`scripts/create-rich-menu.ts`（リッチメニュー作成）。`src/line/api.ts` にブロードキャストとクォータ取得を足した。

#### 出欠は 3 段の postback にした

F4 の保留（どの回への連絡かの特定方法）を回収した。リッチメニュー →（回の選択）→（出欠の選択）の 3 段で、各段がクイックリプライを返す。テキストによる連絡は実装していない。理由は F4 に書いた。

`data` はクライアント由来で改竄されうるため、`lesson_id` は形式（`/^[0-9]+$/`）と実在を確認し、`status` は対訳表のキーであることを確認する。受講生の特定は `event.source.userId` からのみ行い、`data` に `student_id` を載せない。レビューで `student_id=999` を混ぜた postback を実際に投げ、書き込まれるのが検証済みの ID であることを確認した。

#### 確定応答を DB の書き込み結果に依存させない

応答トークンは 1 回限りで、Webhook は 2 秒で timeout する。「9/20 第一回を欠席で承りました」という文面は `lessons` の 1 行と対訳表だけで組み立てられ、`attendances` への書き込み結果を必要としない。そこで応答を先に返し、`upsertStudent` と UPSERT は `waitUntil()` に逃がした。

代償として、書き込みが失敗すると「承りました」と届いたのに DB に無い状態がありうる。画像保存が既に受け入れている割り切りと同じ性質なので許容した。

#### 出欠一覧は students を基準に LEFT JOIN する

誰が連絡していないかが分からないと運営で使えないため、`students` を基準にして出欠行の無い受講生も「未連絡」として出す。絞り込み条件を `WHERE` ではなく `ON` 句に置く必要がある。`WHERE` に置くと未連絡の行が消えて要件が壊れる。

修了した受講生も絞り込まない。過去回の出席記録が見えなくなるため。`note` は今回書き込む経路が無い（クイックリプライだけでは値が入らない）が、将来の自由入力に備えて列だけ表示している。

#### お知らせ配信は確認画面を挟む

ブロードキャストは課金対象で取り消せない。通数は「リクエスト数 × 受信者数」で数えるため、1 回の配信で友だちの人数ぶん消費する（6 章）。

入力 → 確認画面 → 送信の 2 段にし、確認画面に本文・課金の数え方・当月の送信済み通数・取り消せないことを出す。送信に成功したときだけ `notices` に記録する。先に記録すると、送信に失敗したときに「送っていないお知らせ」が履歴に残る。

#### リッチメニューは使い捨てスクリプトで作る

リクエストごとには不要なので Workers 本体に含めず、`scripts/create-rich-menu.ts` をローカルの Node で 1 度実行する形にした。画像は一度設定すると差し替えられないため、作り直しを前提にしている。

画像アップロードだけホストが `api-data.line.me` で、段階 5 のコンテンツ取得と同じ罠がある。ホストを定数に分けてコード上で見分けられるようにした。

右半分の「写真を送る」は `uri` アクションで `https://line.me/R/nv/camera/` を開く。カメラアクション（`type: camera`）はクイックリプライ専用でリッチメニューには使えない。

#### スクリプトの誤実行で本番に反映した

レビュー中に `npm run --silent richmenu:create --dry-run` が実行された。`--dry-run` は `npm run` に対して実行を抑止するフラグではなく、スクリプトへ素通りして無視されるため、実 API に 3 リクエストが飛び、リッチメニューが全友だちのデフォルトになった。

削除して復旧済み。実行前の既存メニューが 0 件だったため失われた資産はなく、リッチメニューの作成・設定は無料なので課金影響もない。ただし postback を処理するコードは当時まだ本番に無く、押しても無反応のボタンが一時的に見えていた。

対処として、`--yes` を明示しない限り何もせず終了する実行ガードを入れた。`--yes` 以外の引数はすべて拒否する（「無視して続行」の余地を残さない）。あわせて順序を「作成 → アップロード → デフォルト設定 → 旧メニュー削除」に変えた。削除が先だと、後続が失敗したときリッチメニューが 1 つも無い状態で残る。

ガードが効くことは、トークンを環境から外した状態で確認した。事故を起こしたコマンドそのものが拒否され、`--yes` を付けた場合のみガードを通過して API 到達前にトークン欠如で止まる。

#### 確認できたこと

統合レビューで、ローカルに本番トークンを載せない状態で次を確認した。

- リッチメニューが送る `action=attendance` が webhook の分岐を正しく通り、3 段すべてで 200 と D1 への書き込みに至る
- 出欠行を持たない受講生が「未連絡」として並ぶ。修了者も含まれる
- 同じ回で出欠を選び直すと UPDATE になり行が増えない。`note` は保持される
- `lesson_id` に空文字・非数値・実在しない値を与えても 200 で DB が汚れない
- お知らせ本文は 2000 文字が通り 2001 文字が 400 で弾かれ、400 のとき入力が差し戻される
- クォータ取得に失敗しても画面は 200 で表示される
- 管理画面 5 画面が表示され、既存の webhook・LIFF・画像配信が壊れていない
- インデックスは段階 7 から変わっていない（マイグレーションを追加していない）

#### 確認できていないこと

実機確認が残っている領域は次のとおり。いずれも LINE アプリか Access 配下の画面が要る。

- リッチメニューの実作成と表示、ボタンを押したときのクイックリプライの見え方
- お知らせの実配信と受信。**送信成功時のみ記録する経路は一度も実行していない**（静的な確認にとどまる）
- 本番のクォータ取得が成功したときの表示
- `/admin/attendances` と `/admin/notices` が未認証で Access へ 302 すること

#### 段階 9 以降への申し送り

段階 8 をもって F1〜F5 の要件をすべて満たし、当初の設計にある機能は作り切った。以降は改善タスクになるため、**申し送りを設計書に積むのをやめて GitHub Issues へ移した**。

段階 4・6・7・8 に散っていた未対応の項目を棚卸しし、Issue #24〜#31 として起票済み。設計書の各段階の申し送りは当時の記録としてそのまま残すが、**これから追うのは Issues のほう**。

段階 8 の時点で残していた 3 件の行き先は次のとおり。

| 内容 | Issue |
| --- | --- |
| `images.received_at` のインデックス欠如（段階 7 からの持ち越し） | #26 |
| `attendances.note` に書き込む経路が無い | #28 |
| お知らせ配信の CSRF トークン | #27 |
...
| LIFF URL の形式 | `https://liff.line.me/{liffId}`。後ろに足したパスとクエリはエンドポイント URL のパスの後ろに連結される（置き換えではない）。クエリは `&` でマージされる | F1 / 7 章 段階 4 |
| LIFF の 2 段階リダイレクト | LIFF URL の追加情報は `liff.state` に畳まれてエンドポイント URL へ渡り（1 次）、そこで実行された `liff.init()` が展開して本来の URL へ飛ばす（2 次）。2 段目を行うのはブラウザ上の SDK であってサーバーではない | 7 章 段階 6 |
| `liff.init()` の前にリダイレクトしない | `init()` の Promise が resolve する前に 301 / 302 を返すと LIFF アプリを正常に開けない（公式が明示）。エンドポイント URL は必ず `init()` を実行する HTML を返す | 7 章 段階 6 |
| `liff.init()` の動作範囲 | エンドポイント URL と完全一致、またはそれより下の階層でのみ動作する | 7 章 段階 6 |
...
| Access のワイルドカードの及ぶ範囲 | 登録済みの `/admin/*` は、後から生やした `/admin/images` とその配下 `/admin/images/{uuid}` も覆う。宛先の追加は不要（実機の 302 で確認） | 7 章 段階 7 |
| `<textarea maxlength>` の効き方 | ブラウザ側で貼り付けを切り詰めるため、通常操作ではサーバーの長さ検証に到達しない。数え方は `String.prototype.length` と同じ UTF-16 コード単位で、両者にずれはない | 7 章 段階 7 |
| メッセージの通数の数え方 | 「リクエスト数 × 受信者数」。ブロードキャスト 1 回は「1 通」ではなく友だちの人数ぶん。ブロック中のユーザーと存在しない ID は対象外でカウントもされない | 6 章 / F5 |
| 送信済み通数の取得 | `GET /v2/bot/message/quota/consumption` が当月の `totalUsage` を返す。上限は `GET /v2/bot/message/quota` | 6 章 / F5 |
| リッチメニュー画像の要件 | 幅 800〜2500px、高さ 250px 以上、アスペクト比（幅÷高さ）1.45 以上、1MB 以下、JPEG または PNG。標準テンプレートは大 2500×1686 / 小 2500×843 | 7 章 段階 8 |
| リッチメニュー画像のホスト | アップロードは `api-data.line.me`。作成・一覧・削除・デフォルト設定は `api.line.me`。コンテンツ取得と同じ罠 | 7 章 段階 8 |
| リッチメニュー画像の差し替え | 一度設定した画像は置き換えられない。変更にはリッチメニューごと作り直す | 7 章 段階 8 |
| リッチメニューのカメラ起動 | カメラアクション（`type: camera`）はクイックリプライ専用。リッチメニューでは `uri` アクションで `https://line.me/R/nv/camera/` を開く（`line://` は非推奨） | F4 / 7 章 段階 8 |
| 各種の上限 | postback の `data` は 300 文字、クイックリプライの `items` は 13 件、ブロードキャストの `messages` は 5 件、`chatBarText` は 14 文字、`areas` は 20 件、`label` は 20 文字 | F4 / F5 |
| reply とクイックリプライ | 応答メッセージにもクイックリプライを付けられる。reply は課金対象外のまま | F4 |
| LINE 公式ドキュメントの読み方 | `developers.line.biz` は SPA で WebFetch では本文が取れない。ソースリポジトリ `line/line-developers-docs-source` の生 Markdown か、OpenAPI 定義 `line/line-openapi` の `messaging-api.yml` を直接読む | 7 章 段階 8 |

R2 は非公開のままにし、Workers 経由でのみ配信する。6 章の画像配信の方針（URL に UUID を使い、推測できないようにする）と整合する。

LIFF の初期化手順は段階 4 で、Webhook と応答メッセージの仕様は段階 5 で、LIFF の URL 解決（2 段階リダイレクトと `liff.state`）は段階 6 で、Access のワイルドカードの及ぶ範囲と `maxlength` の効き方は段階 7 で、リッチメニューと通数の数え方は段階 8 で確認した（7 章）。いずれも 8 章の表に追記済み。

段階 8 の項は実装前に公式の一次情報で裏を取った。段階 6 で「実機で想定と違う挙動に当たってから調べた」失敗をしているため、順序を逆にしている。

表の項目とは別に、実機で動かせていない箇所が 1 つある。メモの長さ上限のサーバー側検証で、理由は 7 章 段階 7 に書いた。

段階 6 の LIFF の項は、実機で想定と違う挙動に当たってから公式ドキュメントで裏を取った。先に確認していれば避けられた手戻りだった。
